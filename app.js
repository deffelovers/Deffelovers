...
app.get('/api/v1/mal', async (req, res) => {
  const mal = (req.query.search == undefined || req.query.search == '') ? { succes: false, msg: 'Bad Request.' } : {
    success: true, note: 'Data ini di ambil dari: https://www.myanimelist.net', data: await require('axios').get(`https://myanimelist.net/search/prefix.json?type=anime&keyword=${req.query.search}`).then(res => res.data.categories[0].items.map(e => {
      return {
        id: e.id,
