    ...
    $.get(`/api/v1/mal?search=${document.querySelector(".input-group input#mal-input").value}`, function(res){
      if(res.success == true){
        ...
        for (let i = 0; i < res.data.length; i++) {
          mal_d += `<div><div class="mal-item" style="padding: 5px;"><div class="card text-white"><img src="${res.data[i].thumb}" class="card-img" alt="${res.data[i].title}"></div></div><div class="col" style="padding-top: 10px; padding: 5px;"><a href="${res.data[i].url}" target="_blank" class="btn btn-primary" style="width: 100%; font-size: 14px;">Lihat di <b>MAL</b></a></div></div>`;
        }
