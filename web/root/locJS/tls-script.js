$(document).ready(function() {
  ...
  $(".input-group button#mal-btn").click(function(){
    ...
    $.get(`/api/v1/mal?search=${document.querySelector(".input-group input#mal-input").value}`, function(res){
      ...
      }else document.querySelector(".input-group input#mal-input").value = document.querySelector(".input-group input#mal-input").defaultValue; document.querySelector('.card div.mal-data').innerHTML = `<i class="text-muted">${res.msg}</i>`; document.querySelector(".input-group button#mal-btn i.bi").setAttribute("class", "bi bi-search"); document.querySelector(".input-group button#mal-btn").classList.remove('disabled');
    })
  })
});